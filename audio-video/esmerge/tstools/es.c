...
ssize_t  read_raw(ES_p es, char *buffer, int len)
{
    struct h264_raw *entry = NULL;
    ssize_t size = 0;

    if (list_empty(&es->head)) {
        return 0;
    }

    entry = list_first_entry(&es->head, struct h264_raw, list);
    if(entry == NULL) {
        return 0;
    }       
    ...
    memcpy(buffer, entry->data+entry->position, size);
    entry->position += size;
...
static inline int get_more_data(ES_p  es)
{
    if (es->reading_ES)
    {
        ...
        if(es->input == -1) {           
            len = read_raw(es,&es->read_ahead,ES_READ_AHEAD_SIZE);
        }
        else
            len = read(es->input,&es->read_ahead,ES_READ_AHEAD_SIZE);
#endif
        if (len == 0)
            return EOF;
        else if (len == -1)
